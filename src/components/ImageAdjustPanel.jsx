...
const ImageAdjustPanel = ({ contrast, setContrast, minLevel, setMinLevel, maxLevel, setMaxLevel }) => (
  <div className="space-y-3 p-4 bg-[var(--bg)] rounded-sm border border-[var(--border)]">
    ...
    <div className="flex items-center gap-4">
      <label className="text-[10px] font-semibold text-[var(--text-secondary)] w-16 shrink-0 uppercase tracking-wider">
        Levels
      </label>
      <div className="flex-1 flex gap-2 min-w-0">
        <input
          ...
          className="flex-1 min-w-0 h-1 bg-[var(--border)] rounded appearance-none cursor-pointer accent-[var(--accent)]"
        />
        <input
          type="range"
          min="0" max="255" step="1"
      ...
      <div className="flex flex-col text-[10px] w-8 shrink-0 text-right leading-3 font-mono text-[var(--text-secondary)]">
        <span>{maxLevel}</span>
